...
以下是一个**复制粘贴**示例，名为 `crew-quickstart.tsx`。将此代码放入您的代码后，只需在您的应用程序中渲染 `<QuickstartCrew />` 即可看到它的实际效果！
...
export const useCrewQuickstart = ({
  ...
  const instructions =
    "输入是绝对必需的。请在执行其他任何操作之前调用 getInputs。";
