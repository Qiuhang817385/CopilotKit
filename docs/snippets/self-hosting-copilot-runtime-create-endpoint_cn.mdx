...
<Callout type="info">
  如果您计划使用单个 LangGraph 代理作为代理后端（agent-lock 模式），您的 LLM 适配器仅用于建议等周边功能。
...
如果您还不确定，请忽略此说明。
    ...
    <If conditions={[{ key: 'id', value: 'langchain' }]}>
        <Callout type="info">
            此处展示的 LangChain 适配器使用 OpenAI，但可以与任何 LLM 配合使用！
        </Callout>
    </If>

    <If conditions={[{ key: 'id', value: 'empty' }]}>
        <Callout type="error">
            请注意，空适配器仅在与 CoAgents 的 agent lock 模式结合使用时才有效！
    ...
    <If conditions={[{ key: 'envVarName', value: true }]}>
        ...
        接下来，将您的 API 密钥添加到项目根目录的 `.env` 文件中（除非您希望直接将其提供给客户端）：
    ...
    <Callout type="warn">
        ...
        部署到无服务器平台（Vercel、AWS Lambda 等）时，请注意默认的函数超时时间可能对于 CopilotKit 的流式响应来说太短：

        - Vercel 默认值：10秒（Hobby）、15秒（Pro）
        - AWS Lambda 默认值：3秒
    ...
    <Tabs groupId="endpoint-type" items={[
        ...
        <Tab value="Next.js App Router">
            ...
            您的 Copilot 运行时端点应该在 `http://localhost:3000/api/copilotkit` 可用。
        </Tab>
        ...
        <Tab value="Node.js Express">
            创建一个新的 Express.js 应用并设置 Copilot 运行时处理程序：
            ...
            您的 Copilot 运行时端点应该在 `http://localhost:4000/copilotkit` 可用。

            <Callout type="warn">
                请记住在客户端代码中将 `runtimeUrl` 指向正确的端点，例如 `http://localhost:PORT/copilotkit`。
            </Callout>
        </Tab>

        {/* Node.js HTTP */}
        <Tab value="Node.js HTTP">
            设置一个简单的 Node.js HTTP 服务器并使用 Copilot 运行时处理请求：
        ...
        {/* NestJS */}
        <Tab value="NestJS">
            在 NestJS 中设置一个控制器来处理 Copilot 运行时端点：
            ...
            您的 Copilot 运行时端点应该在 `http://localhost:3000/copilotkit` 可用。
