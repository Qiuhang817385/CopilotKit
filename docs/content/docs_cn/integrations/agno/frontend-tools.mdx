...
title: 前端工具
icon: "lucide/Wrench"
description: 创建前端工具并在您的 Agno Agent 中使用它们。
---
...
## 这是什么？
前端工具使您能够定义客户端函数，供您的 Agno Agent 调用，执行完全在用户的浏览器中进行。当您的 Agent 调用前端工具时，
逻辑在客户端运行，让您可以直接访问前端环境。

这可以用于[让您的 Agent 控制 UI](/agno/frontend-tools)、[生成式 UI](/agno/frontend-tools) 或[人机协同](/agno/human-in-the-loop)交互。
...
<Steps>
    ...
    <Step>
        ...
        首先，您需要使用 [useFrontendTool](/reference/v1/hooks/useFrontendTool) Hook 创建一个前端工具。这里有一个简单的示例，
        向用户问好。
    ...
    <Step>
        ### 在 Agno Agent 中定义前端工具

        现在，我们需要修改 Agent 以访问这些前端工具。

        在您的 Agno Agent 中，使用 `@tool(external_execution=True)` 装饰器定义一个工具。这告诉 Agno 该工具将在外部（在前端）执行。
    ...
    <Step>
        ### 试试看！
        您现在已经赋予您的 Agent 直接调用您定义的任何前端工具的能力。这些工具将对 Agent 可用，可以在需要时使用。
