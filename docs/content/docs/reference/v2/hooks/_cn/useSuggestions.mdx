...
`useSuggestions` 是一个 React Hook，提供对给定 Agent 的当前聊天建议的访问。它订阅建议更改，并在建议更新、清除或重新加载时自动重新渲染。
...
<PropertyReference name="options" type="UseSuggestionsOptions">
  ...
  <PropertyReference name="agentId" type="string" default='"default"'>
    要检索其建议的 Agent ID。如果省略，将回退到最近的 `CopilotChatConfigurationProvider` 中的 Agent ID，或 `"default"`。
  </PropertyReference>
</PropertyReference>
...
<PropertyReference name="result" type="UseSuggestionsResult">
  ...
  <PropertyReference name="suggestions" type="Suggestion[]">
    ...
    - `message: string` -- 选中建议时发送的消息内容。
    - `isLoading: boolean` -- 此建议是否仍在生成中。
  </PropertyReference>

  <PropertyReference name="reloadSuggestions" type="() => void">
    触发已解析 Agent 的建议重新加载。对于动态建议，这会重新调用生成过程。`isLoading` 状态通过订阅事件自动更新。
  </PropertyReference>
...
- **自动订阅**：Hook 订阅核心实例上的建议更改事件，并在已解析 Agent 的建议添加、移除或更新时重新渲染。
- **Agent 解析**：`agentId` 按以下优先顺序解析：显式 `agentId` 选项、最近的 `CopilotChatConfigurationProvider` 中的 `agentId`，然后是默认 Agent ID (`"default"`)。
