---
title: "LangGraph SDK"
description: "适用于 Python 的 CopilotKit LangGraph SDK 允许你使用 CopilotKit 构建和运行 LangGraph 工作流。"
---
...
运行完成后退出当前 Agent。调用 copilotkit_exit() 不会
...
向 CopilotKit 发出中间状态。如果你有一个运行时间较长的节点，并且想要
    向用户更新节点的当前状态，这会很有用。
...
手动向 CopilotKit 发出消息。在运行时间较长的节点中更新用户时很有用。
    重要：你仍然需要从节点返回消息。
