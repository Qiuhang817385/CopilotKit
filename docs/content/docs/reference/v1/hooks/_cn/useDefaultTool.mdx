...
description: "useDefaultTool 钩子启用默认 UI 的渲染，捕获任何没有特定渲染器的工具。"
---

<Callout type="warning">
  `useDefaultTool` 仍然受支持，但我们建议迁移到 v2 API 的 [`useFrontendTool`](/reference/v2/hooks/useFrontendTool)。
</Callout>

`useDefaultTool` 是一个 React 钩子，允许您为任何没有特定渲染器的工具调用渲染自定义 UI
