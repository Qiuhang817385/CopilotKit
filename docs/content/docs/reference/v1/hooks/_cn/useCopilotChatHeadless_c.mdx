...
<Callout type="warning">
  `useCopilotChatHeadless_c` 仍然受支持，但我们建议迁移到 v2 API 的 [`useAgent`](/reference/v2/hooks/useAgent)。
</Callout>

`useCopilotChatHeadless_c` 用于构建完全自定义 UI（无头 UI）实现。
 
<Callout title="这是一个高级功能">
在 [Copilot Cloud](https://cloud.copilotkit.ai) 上免费注册以获取你的公共许可证密钥，或阅读更多关于<a href="/premium/overview">高级功能</a>的信息。
 
使用量慷慨，免费开始使用，并且适用于自托管或 Copilot Cloud 环境。
</Callout>
 
## 主要功能
 
- 完全无头：为你的代理应用程序构建你自己的完全自定义 UI。
- 高级建议：直接访问建议数组，完全控制
- 中断处理：支持高级中断功能
